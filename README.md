...
- **Sesi login dibagikan antar-tab.** Di mode demo, status login (siapa yang
  sedang aktif) disimpan di `localStorage`, yang otomatis dibagikan ke
  **semua tab** pada browser yang sama. Kalau kamu membuka beberapa tab
  sekaligus dan login akun berbeda di tiap tab, tab-tab itu akan saling
  menimpa sesi satu sama lain — transaksi yang disimpan akan tercatat atas
  nama akun yang **terakhir login**, bukan akun yang terlihat aktif di tab
  itu. **Cara aman menguji multi-akun: login → tambah transaksi → logout →
  baru login akun berikutnya, satu tab, satu per satu** — jangan login
  beda akun di beberapa tab bersamaan.
  - Untuk membantu mendeteksi ini, form "Tambah Transaksi" sekarang
    menampilkan badge **"Transaksi ini akan dicatat sebagai kasir: [nama]"**
    — selalu cek nama di badge ini sebelum menyimpan.
  - Sistem juga otomatis mendeteksi jika sesi berubah di tab lain saat form
    transaksi sedang terbuka, dan akan menutup form tersebut demi keamanan.
  - Batasan ini **hilang total di mode Supabase** — di sana setiap
    device/browser punya sesi login sendiri-sendiri lewat Supabase Auth,
    jadi beberapa kasir bisa login bersamaan di perangkat masing-masing
