...
function renderSummary(transactions, products) {
  ...
  const totalHPP = lunasTrx.reduce((sum, t) => {
    const p = productMap.get(t.productId);
    return p ? sum + p.buyPrice * t.qty : sum;
  }, 0);
  const netProfit = lunasTrx.reduce((sum, t) => {
    const p = productMap.get(t.productId);
    if (!p) return sum;
  ...
  const hppEl = document.getElementById('sumHPP');
  if (hppEl) hppEl.textContent = formatCompactRupiah(totalHPP);
}
  ...
  tbody.innerHTML = products.length ? products.map((p) => `
    <tr data-id="${p.id}">
      ...
      <td class="action-cell">
        <button class="row-action-btn restock" data-action="restock" title="Restock">📥</button>
...
/* --- Navigasi antar-view --- */
function initNav() {
  const links = document.querySelectorAll('.nav-link, .bottom-nav-link[data-view]');
  links.forEach((link) => {
  ...
  document.getElementById('bottomNavFab')?.addEventListener('click', (e) => {
    e.preventDefault();
...
function switchView(target) {
  document.querySelectorAll('.nav-link, .bottom-nav-link[data-view]').forEach((l) => l.classList.toggle('active', l.dataset.view === target));
...
function applyRoleRestrictions() {
  ...
  document.querySelectorAll('.nav-link[data-view], .bottom-nav-link[data-view]').forEach((link) => {
...
/* --- Kelola Profil Owner (nama toko, nama pemilik, password) --- */
function initOwnerProfileForm() {
  const form = document.getElementById('ownerProfileForm');
  if (!form) return;

  const storeInput = document.getElementById('ownerStoreName');
  const nameInput = document.getElementById('ownerFullName');
  if (storeInput) storeInput.value = getStoreName();
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!storeName || !fullName) { showToast('Nama toko & nama pemilik wajib diisi.', 'warning'); return; }
    if (newPassword && newPassword !== confirmPassword) { showToast('Konfirmasi password baru tidak cocok.', 'warning'); return; }
    if (newPassword && newPassword.length < 6) { showToast('Password baru minimal 6 karakter.', 'warning'); return; }

    submitBtn.disabled = true;
    submitBtn.textContent = 'Menyimpan...';
    try {
      await updateOwnerProfile({ fullName, storeName, newPassword: newPassword || null });
      ...
      document.getElementById('userInitial2').textContent = fullName.charAt(0).toUpperCase();
