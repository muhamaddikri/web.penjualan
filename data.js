...
async function getTransactions() {
  if (APP_CONFIG.USE_SUPABASE) {
    ...
    return data.map((t) => ({
      id: t.id, trxCode: t.trx_code, date: t.trx_date, customer: t.customer, productId: t.product_id,
...
async function addTransaction(trx) {
  ...
  if (APP_CONFIG.USE_SUPABASE) {
    const { error: trxError } = await supabaseClient.from('transactions').insert({
      trx_code: trx.trxCode, trx_date: trx.date, customer: trx.customer, product_id: trx.productId,
      product_name: product.name, category: product.category, qty: trx.qty, total: trx.total,
      payment_method: trx.paymentMethod, status: trx.status,
    });
