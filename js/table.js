...
function renderTable() {
  ...
  if (pageRows.length === 0) {
    ...
    tbody.innerHTML = pageRows.map((t) => `
      <tr data-id="${t.id}" ${owner ? 'title="Klik dua kali untuk lihat detail lengkap"' : ''}>
    ...
    if (owner) {
      ...
      // Fitur khusus Owner: double-click baris untuk melihat detail lengkap transaksi
      tbody.querySelectorAll('tr[data-id]').forEach((row) => {
        row.addEventListener('dblclick', () => {
          const id = row.dataset.id;
          const trx = tableState.all.find((t) => t.id === id);
          if (trx && typeof openTrxDetailModal === 'function') openTrxDetailModal(trx);
        });
      });
    }
  }
